...
def test_verify_rejects_tampered_copy_projection(
    ...
    with pytest.raises(HarnessError, match="modified"):
        project_mode(workspace, project, "creator-studio", host_id="codex-app")
    assert "tampered" in (project / ".agents/skills/creator/SKILL.md").read_text(encoding="utf-8")
...
def test_exports_self_contained_deepseek_agent_preset(tmp_path: Path) -> None:
    ...
    configured = json.loads((output / "asl-hooks.json").read_text(encoding="utf-8"))
    command = configured["hooks"]["SessionStart"][0]["hooks"][0]["command"]
    assert "--preset" in command and str(output) in command
...
def test_mode_access_ignores_unrelated_incomplete_material(tmp_path: Path, capsys) -> None:
    root = _environment(tmp_path)
    _write(root / "skills/unrelated/SKILL.md", "unfinished")
    _write(root / "trials/draft/SKILL.md", "unfinished")
    _write(root / "candidates/idea/note.md", "not ready")
    with pytest.raises(HarnessError):
        Workspace.open(root)
    project = tmp_path / "project"
    arguments = ["--workspace", str(root), "--project", str(project),
                 "--mode", "creator-studio", "--host-id", "claude-code"]
    assert main(["host.project", *arguments]) == 0
    capsys.readouterr()
    code, message = run_hook({"hook_event_name": "SessionStart", "cwd": str(project)},
                             host_id="claude-code")
    assert code == 0 and "2 projected Skills" in message
    _write(root / "skills/creator/.env", "placeholder")
    assert main(["host.verify", *arguments]) == 2
    assert "SECRET_FILE_PRESENT" in capsys.readouterr().out


def test_deepseek_preset_hook_works_from_an_unrelated_project(tmp_path: Path, monkeypatch, capsys) -> None:
    import io
    from asl_harness.hooks import main as hook_main
    workspace = Workspace.open(_environment(tmp_path))
    preset = tmp_path / "presets/creator-studio"
    export_preset(workspace, "creator-studio", _deepseek_base(tmp_path / "base"), preset)
    unrelated = tmp_path / "ordinary-project"
    unrelated.mkdir()
    payload = json.dumps({"hook_event_name": "SessionStart", "cwd": str(unrelated)})
    monkeypatch.setattr("sys.stdin", io.StringIO(payload))
    assert hook_main(["--host-id", "deepseek-harness", "--preset", str(preset)]) == 0
    assert "2 projected Skills" in capsys.readouterr().out
    code, message = run_hook({"hook_event_name": "SessionStart", "cwd": str(preset)},
                             host_id="deepseek-harness")
    assert code == 0 and "2 projected Skills" in message


@pytest.mark.parametrize("replacement", [
    ("includeDefaultRoots: false", "includeDefaultRoots: true"),
    ("ASL Mode creator-studio", "unrelated persona"),
])
def test_preset_rejects_changed_mode_configuration(tmp_path: Path, replacement) -> None:
    workspace = Workspace.open(_environment(tmp_path))
    preset = tmp_path / "presets/creator-studio"
    export_preset(workspace, "creator-studio", _deepseek_base(tmp_path / "base"), preset)
    path = preset / "agent.cordis.yml"
    before = path.read_text(encoding="utf-8")
    assert replacement[0] in before
    _write(path, before.replace(*replacement))
    with pytest.raises(HarnessError, match="configuration"):
        verify_preset(workspace, "creator-studio", preset)


def test_git_only_changes_do_not_require_reprojection(tmp_path: Path) -> None:
    ...
    project_mode(workspace, project, "creator-studio", host_id="codex-app")
    export_preset(workspace, "creator-studio", _deepseek_base(tmp_path / "base"), preset)
    updated = replace(workspace, git_commit="a" * 40)
    assert verify_mode_projection(updated, project, "creator-studio", host_id="codex-app") == []
    assert verify_preset(updated, "creator-studio", preset) == []


def test_harness_does_not_override_skill_reading_rules(tmp_path: Path) -> None:
    from asl_harness.deepseek import _mode_persona
    workspace = Workspace.open(_environment(tmp_path))
    for text in [adapters._mode_instructions(workspace, "creator-studio"),
                 _mode_persona(workspace, "creator-studio")]:
        assert "package fully" not in text
        assert "full package before" not in text
        assert "No directory named Case" in text


def test_runtime_notes_in_source_are_not_missed(tmp_path: Path) -> None:
    root = _environment(tmp_path)
    source = root / "skills/creator/SOURCE.md"
    _write(source, source.read_text(encoding="utf-8") + "\n- Runtime dependencies: Python, opencli.\n")
    report = adapters.activation_report(Workspace.open(root), tmp_path / "project",
                                        "creator-studio", host_id="codex-app")
