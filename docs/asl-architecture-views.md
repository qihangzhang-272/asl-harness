...
flowchart TB
    ...
    subgraph ENV["用户本地 Environment · 当前运行与维护内容，可用 Git 管理"]
        ...
        MAP["Mode 技能架构 · 本机呈现已验证<br/>一个完整 Skill 一个节点 · Mermaid 原生图<br/>本地节点显示与关联 → 安全投影；App / Agent 同源编辑"]
        ROOT --> MODES
        MODES -->|显式选择能力| SKILLS
        ROOT --> CONTEXT
        SKILLS --> MAP
...
**当前定义：一个 Skill 就是一个节点。** 架构图呈现 Mode 已沉淀的技能组织关系，不是操作教程，也不是强制工作流。不要求填写“如何工作”、触发条件、操作步骤或输入输出。图中不增加 Mode 根节点、类别节点、场景模块或技能内部步骤；一个完整技能只出现一次，多个关联可以连接同一节点。Mode 的名称与说明放在图外。
...
| `spec.architecture.edges`（可选） | `from`、`to` 直接引用技能 ID，`label` 为可选关系说明 | 两端属于当前 Mode、不同且有序对不重复；允许成环，不把它当运行循环。移出成员时去掉失效显示覆盖及关联，不删除 Skill 包 |
...
不再保留 `spec.presentation.layout` 或两套图类型配置；架构与成员继续保存在同一份 `mode.yaml`。编辑可选，不要求用户先画图才能使用 Mode。

这些定义随 Mode 导入、导出和复制，不锁在 App 偏好中。普通 App 编辑先通过同一核心检查；不合法时给出错误并保留输入。直接改文件后，打开的 App 监听当前环境的 `modes/`、`skills/` 与 `PROFILE.md`，合并短时间通知并重新读取；不做全盘扫描，不修改文件。编辑弹窗打开时延后刷新，原文件已改变则拒绝旧草稿覆盖。直接写入了无效结构时保留上次有效界面并提示修正；没有把错误文件伪装成已经生效。App 关闭后没有额外常驻监听服务。
...
| 技能分类中的研究、写作、视觉等分组 | 默认 `presentation.mjs` 按名称 / 标识优先、说明其次建议分组；用户可保存 `mode.yaml spec.capabilities`，之后以手动类别为准 | 建议分类不是 AI 语义匹配，不生成架构节点，不改变 Mode 成员；类别随 Mode 分享，没有额外清单文件 |
...
发布前发现 Mermaid 间接使用的 `lodash-es 4.17.23` 存在已知安全通告，使用 `overrides` 固定到 `4.18.1`，不降级或重写图形组件；重新安装后 `npm audit` 已知漏洞数为 0。CI 新增高危依赖检查与只读仓库权限，Git 忽略本地 `.env` 和 `.local`。这不是全面安全认证：原生权限仍归宿主，应用仍未签名。

0.2.1 本地验证：Python 117 项通过、3 项平台差异跳过；桌面 50 项通过；重新冻结并打开 App，当前版本号、提示词、分支关系图及无关系图均已检查，无页面错误。远端检查发现并修复测试资料的默认编码问题，以及 Windows 短路径触发的底层文件监听崩溃；监听前转换为原生长路径，新增回归测试，并在最终便携程序中用真实短路径打开测试副本、修改内容、确认界面自动刷新。未通过跳过检查来发布。

[Windows 预览版 app-v0.2.1](https://github.com/qihangzhang-272/asl-harness/releases/tag/app-v0.2.1) 已公开；对应提交 `84a7911aa613d5689d1f2582bb2fb922a7cce42d` 的 [Windows / Linux CI](https://github.com/qihangzhang-272/asl-harness/actions/runs/34692092374) 全部通过。ZIP 含 383 个文件、174,358,235 字节，逐文件哈希及 ZIP 完整性检查通过；SHA-256 为 `c872eeb703486eead53db608d2b4afa297525a6964602d86ac9e9c1a3db6d247`，与 GitHub 附件摘要相同。源码只同步 Git 跟踪文件，不同步个人环境、原生用户配置或实验目录。经用户确认正常关闭旧窗口，本机已打开最终新版并沿用原偏好；桌面快捷方式不再指向临时构建目录。

**正式库与测试图的边界：**本轮只读核对发现，实际 Agent Skill Library 的 4 个 Mode 尚未写入 `spec.architecture`；此前 Creator Studio 的 19 节点 / 23 关联演示来自隔离测试副本。没有关联定义时，App 仍会绘制已有技能节点，因此“图出现”不代表已保存或恢复那份关联。图的持久内容应写在各 Mode 的 `mode.yaml`；App 偏好只记住打开的工作环境、模式和视图。尚未把测试关系迁入正式库，也不将它们冒充用户已认可的工作架构。

### 本机建立入口与重开恢复（2026-09-12，便携版已验证）

已确认的首选路径是“从这台电脑建立工作环境”；外部导入作为第二入口保留，先以 Agent Skill Library 验证，不扩建市场。Mode 按工作目的和场景划分，不按个人身份划分。以下为 **0.2.0 历史验证**，该本地旧包已由上方 0.2.1 取代，不再作为推荐交付；当时在独立偏好目录实测，未修改真实技能库、个人工作区或用户宿主配置。

- `desktop/library.cjs`、`main.cjs`、`App.jsx`：在现有偏好文件保存各环境的模式、页面、视图、选中技能、搜索及发现来源。开发版重开恢复 Creator Studio / 技能列表 / 技能详情；便携版重开恢复 Product Lab / 技能列表；单实例重复启动退出第二进程并保留同一窗口。无效旧选择回退与多环境隔离有自动测试。发现结果重开时重新解析，不保存整库快照；未保存编辑草稿不恢复。
- 本机建立以提示词作为开放接口：App 给出当前本地目录、已有内容和编辑规范，用户复制给自己正在用的 AI；AI 整理本地文件后，App 刷新“我的模式”。不启动 CLI、不在后台运行模型，不创建整理任务调度器。原生配置助手不承担这条入口。
- 首屏和已有环境的提示词已实测复制：明确按工作目的组织、完整技能为节点、用户选定参考范围和随包核心。空环境取提示词不创建业务文件。用真实库的隔离副本模拟本地内容落入后，App 读出 4 Mode / 37 Skill；直接修改副本 Mode 标题，界面自动回读。不是实际模型自动生成模式的验收。
- Python **117 项通过、3 项跳过**，桌面 **49 项通过**；Vite 与 Windows 冻结核心构建通过，冻结核心中文 / emoji 读写通过。便携版 Mermaid 图和整理弹窗已截图检查。最小窗口重测因自动化连接超时未完成，不冒充已通过。
- 新包只包含程序、随包核心、公开内置示例与许可；测试目录、个人技能、账号不入包。此前 v17 程序不会自动升级，也没有新单实例保护；使用新版前应正常关闭旧窗口。
- 历史本地包 `ASL-Workspace-Windows-0.2.0.zip`：383 个文件、174,358,688 字节；逐文件 SHA-256 与打包目录一致，ZIP CRC 检查通过。包 SHA-256：`7775BBFBF7D37C6FF79E6A39DBD9A875A2280B1D69497E4F058260B446DCF9CD`。总架构 20 个 Mermaid 图语法检查通过。该阶段没有发布，当前远端交付见上方 0.2.1。

回答：哪些已经能用，哪些只是设计完成，下一步怎样把底座变成可传播的 App？

**2026-09-12 当前实施范围：**只推进 ASL Harness；Agent Skill Library、个人工作区及协议仓库不随本轮修改。首选入口更新为本机提示词整理，保留云端 Mode 导入、本地维护、来源更新窗口和标准用户目录；不以自动测试或目录识别替代宿主运行验收。以下历史记录均保留日期，当前增量见上方。
...
**2026-09-10 本轮新增验收：**Python **101 项通过、3 项跳过**，桌面 **35 项通过**，Vite 与 Windows 冻结核心构建通过。开发源和公开发布检出保持单向同步。新功能使用真实 Agent Skill Library 的独立副本验证，原库与个人工作区不改写。针对“能发现但没有添加入口”的反馈，新增跨 Mode 复用 / 显式替换请求检查；开发版与便携版均实测卡片添加、模式选择、保存与取消。
...
| 发现后直接添加 | `taste-skill` 真实仓库的 13 张卡片均有添加按钮；GUI 将 brandkit 加入 Creator Studio 的用户类别，再复用库内版本加入 Product Lab；卡片显示两个 Mode 并可进入能力地图 | 第二次只改目标 Mode，Skill 原文未变化；正式库没有测试写入 |
| 配套内容提示 | 带终端命令提示的 design-taste-frontend 经选择 Mode、预览和确认后保存；Node / Python / MCP 声明仍仅解析、不执行 | 只保存所选 Skill 目录；外部依赖未安装、未宣称运行成功；引用线索可能遗漏或误报 |
...
总架构文档保留原有专项图，2026-09-12 更新 Master、View 2D 和本受管状态区，忠实区分技能架构、分类、完整文件编辑及本地刷新。绿色只表示所标注的代码能力，不表示所有 GUI 路径或宿主会话已验收；图作为派生呈现仍用灰色。
