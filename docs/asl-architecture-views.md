...
当前本机回归：Python 91 项通过、3 项跳过（Windows 符号链接权限与 POSIX 执行位）；桌面测试 25 项通过，新增用户级同步、完整配置材料、原生命令参数隔离与 Windows 实际交接回执测试。Vite 生产构建、20 张 Mermaid 图渲染通过。用户级同步覆盖两宿主的隔离 home、切换 / 停用、非受管同名冲突、受管副本改动、过期预览、自定义目录和失败回滚。原生 App 已实际完成：内置示例 → Codex 当前用户范围 → 查看新增差异 → 系统确认 → 写入隔离用户的两个完整 Skill → 进入配置检查页。文件回读与界面相符，未写真实用户全局 Skills 或真实账号。安装版的项目 / 用户切换、实际目录显示与原生文件夹选择入口已检查，自选目录的写入及停用回归由隔离测试覆盖。此前真实 Agent Skill Library 的 4 Mode / 37 Skill 读取、模式切换与地图核对仍有效，业务内容未改。本轮代码 `c883695` 的 [Windows / Linux CI](https://github.com/qihangzhang-272/asl-harness/actions/runs/34426751930) 均已通过（核心测试、桌面测试、生产构建）。Windows 便携版已打包，185 个归档文件逐项核对一致；不含私人技能和账号数据，不要求预装 Python。已有缓存、归档与用户文件未清理。
