...
0.2.1 本地验证：Python 117 项通过、3 项平台差异跳过；桌面 49 项通过；重新冻结并打开 App，当前版本号、提示词、分支关系图及无关系图均已检查，无页面错误。源码只同步 Git 跟踪文件，不同步个人环境、原生用户配置或实验目录。当前先准备版本草稿，完成远端 CI 和附件校验后再公开。

### 本机建立入口与重开恢复（2026-09-12，便携版已验证）

已确认的首选路径是“从这台电脑建立工作环境”；外部导入作为第二入口保留，先以 Agent Skill Library 验证，不扩建市场。Mode 按工作目的和场景划分，不按个人身份划分。版本 **0.2.0** 已构建并在独立偏好目录实测，未修改真实技能库、个人工作区或用户宿主配置。
...
- 本地交付 `ASL-Workspace-Windows-0.2.0.zip`：383 个文件、174,358,688 字节；逐文件 SHA-256 与打包目录一致，ZIP CRC 检查通过。包 SHA-256：`7775BBFBF7D37C6FF79E6A39DBD9A875A2280B1D69497E4F058260B446DCF9CD`。总架构 20 个 Mermaid 图语法检查通过。此轮尚未推送远端。
...
| 🟠 部分实现 | 独立 App | 0.2.0 Windows 便携版；Electron + React + Mermaid；一个技能一个节点的架构、关联编辑、整包文件编辑、Mode 内增减、提示词整理、本地刷新与重开恢复；保留云端导入、更新和宿主配置 | 真实 Mode 的关系须由人或授权 Agent 维护，不能宣称自动理解；复杂融合、原生 AI 配置成功实测、跨电脑、签名发行仍未完成 |
