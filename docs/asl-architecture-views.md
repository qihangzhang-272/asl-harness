...
当前本机回归：Python **92 项通过、3 项跳过**（Windows 符号链接权限与 POSIX 执行位）；桌面 **30 项通过**；Vite 生产构建通过。新增真实 IPC 取消 / 只读 / 路径回退检查，并将中文加 emoji 的读写加入冻结 EXE 的构建门禁，不只测试开发机 Python。Windows 修正版便携包已构建并以真实用户目录运行；公开包不包含真实技能库、测试库或账号。此前代码 `c883695` 的 [Windows / Linux CI](https://github.com/qihangzhang-272/asl-harness/actions/runs/34426751930) 通过只代表此前版本，本轮远端验证以对应新提交的 CI 为准。
